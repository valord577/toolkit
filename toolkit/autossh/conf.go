...
const autosshConfPath = "TOOLKIT_AUTOSSH_CONF_PATH"

func readConfInFile() error {
	file := os.Getenv(autosshConfPath)
	bs, err := os.ReadFile(strings.TrimSpace(file))
	if err != nil {
		return err
	}
	return json.Unmarshal(common.JsoncIgnoreComments(bs), &c)
}
